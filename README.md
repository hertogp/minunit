...
│   ├── x.c
│   └── x.h
└── tst                    # another temp dir for unit test runners
    ├── test_x_1
    └── test_x_2
...
The following is a complete unit test file:
...
Add more test functions to the file as needed.  Test functions are run in
the order of appearance in the test file.
...
The test macros only complain on failures, which looks like the following:
...
Running `make test` would run both test units.
